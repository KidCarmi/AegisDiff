...
_PYTHON_STDLIB = frozenset({
    ...
    "copy", "enum", "glob", "gzip", "hmac", "http", "html", "logging",
    "random", "shutil", "signal", "socket", "sqlite3", "string", "struct",
    "subprocess", "tempfile", "threading", "traceback", "typing", "unittest",
    "urllib", "uuid", "warnings", "xml", "zipfile", "zlib", "base64",
    "collections", "contextlib", "dataclasses", "datetime", "decimal",
    "functools", "hashlib", "heapq", "inspect", "itertools", "operator",
    "pathlib", "pickle", "platform", "pprint", "queue", "secrets", "stat",
    "textwrap", "types", "weakref", "argparse", "configparser", "difflib",
    "importlib", "locale", "multiprocessing", "numbers", "pkgutil", "runpy",
    "sched", "ssl", "statistics", "tarfile", "unicodedata", "venv", "wave",
    # common third-party (LLM already knows these)
    "flask", "django", "fastapi", "sqlalchemy", "requests", "httpx",
    "pydantic", "celery", "redis", "boto3", "pytest", "click", "aiohttp",
    "starlette", "tornado", "bottle", "peewee", "jwt", "cryptography",
    "numpy", "pandas", "scipy", "sklearn", "tensorflow", "torch",
    "yaml", "toml", "dotenv", "sentry_sdk", "stripe", "twilio",
    "bleach", "markupsafe", "jinja2", "werkzeug", "wtforms",
})

_GO_STDLIB = frozenset({
    "fmt", "os", "io", "net", "log", "math", "sync", "sort", "time",
    "errors", "bytes", "bufio", "regexp", "strings", "strconv", "unicode",
    "runtime", "reflect", "context", "testing", "encoding", "database",
    "html", "http", "url", "path", "filepath", "archive", "compress",
    "crypto", "image", "mime", "text", "hash", "flag", "signal", "syscall",
})

_JAVA_STDLIB_PREFIXES = (
    "java.", "javax.", "sun.", "com.sun.", "org.w3c.", "org.xml.",
    "org.springframework.", "com.fasterxml.", "org.apache.", "org.slf4j.",
    "com.google.", "io.micronaut.", "io.quarkus.",
)

_JS_BUILTINS = frozenset({
    "console", "process", "Buffer", "Promise", "Error", "JSON", "Math",
    "Object", "Array", "String", "Number", "Boolean", "Date", "RegExp",
    "Map", "Set", "Symbol", "Proxy", "Reflect", "globalThis",
})
...
def extract_tainted_function_names(paths: List[DataFlowPath]) -> List[str]:
    ...
    """
    _skip = frozenset({
        "if", "for", "while", "return", "not", "and", "or", "in", "is",
        "None", "True", "False", "len", "str", "int", "list", "dict", "set",
        "tuple", "print", "range", "type", "bool", "float", "bytes", "open",
        "super", "zip", "map", "filter", "sorted", "enumerate", "reversed",
...
def _resolve_python(
    source_code: str, func_names: List[str], file_path: str
) -> Dict[str, str]:
    result: Dict[str, str] = {}
...
def _extract_body(
    source: str, func_name: str, ext: str, max_lines: int
) -> Optional[str]:
    if ext in (".py", ".pyw", ".rb"):
