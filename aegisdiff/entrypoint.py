...
def main() -> None:
    ...
    # ── Build provider list ────────────────────────────────────────────────
    # Priority: OpenRouter llama-3.3-70b:free → OpenRouter deepseek-v3:free
    ...
    for i, key in enumerate(openrouter_keys, start=1):
        providers.append(OpenRouterProvider(key, model="deepseek/deepseek-chat-v3-0324:free"))
        logger.info("Provider: OpenRouter deepseek-v3:free (user key %d/%d)", i, n_or)
    ...
    if oidc and cfg.aegisdiff_ingest_url:
        ...
        for i, key in enumerate(platform_openrouter, start=1):
            providers.append(OpenRouterProvider(key, model="deepseek/deepseek-chat-v3-0324:free"))
            logger.info("Provider: OpenRouter deepseek-v3:free (platform %d/%d)", i, n_por)
