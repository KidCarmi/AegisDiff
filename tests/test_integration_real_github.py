"""
Real-GitHub integration test — live end-to-end PR scan.

Creates an actual PR on a dedicated test repo, injects deliberately vulnerable
code, runs the full AegisDiff engine against it (real LLM, real GitHub App
token), and then verifies:

  ✓ AegisDiff posted a PR comment with a TRUE_POSITIVE verdict
  ✓ A "failure" commit status was set (branch protection works)
  ✓ The scan metadata reached the dashboard /api/ingest endpoint

Setup — one-time (see CONTRIBUTING.md):
  1. Create a dedicated test repo, e.g.  KidCarmi/aegisdiff-integration-target
  2. Install the AegisDiff GitHub App on that repo
  3. Add the secrets below to THIS repo (KidCarmi/AegisDiff):

Required secrets / environment variables
─────────────────────────────────────────
  INTEGRATION_TEST_GITHUB_TOKEN  — PAT with `repo` scope on the test repo
                                   (used to create branches + PRs)
  INTEGRATION_TEST_REPO          — "owner/name"  e.g. KidCarmi/aegisdiff-integration-target
  INTEGRATION_INSTALLATION_ID    — GitHub App installation ID for the test repo
  GITHUB_APP_ID                  — AegisDiff GitHub App numeric ID (already in our secrets)
  GITHUB_APP_PRIVATE_KEY         — RSA private key PEM (already in our secrets)
  GITHUB_TOKEN                   — auto-injected by Actions; used as GitHub Models fallback
  AEGISDIFF_INGEST_URL           — optional: real dashboard ingest URL
  AEGISDIFF_INGEST_TOKEN         — optional: per-repo ingest token

The test is automatically SKIPPED when any required variable is absent
so it is always safe to run in a local dev environment.
"""

from __future__ import annotations

import json
import os
import random
import string
import time
from typing import Optional
from unittest.mock import patch

import httpx
import pytest

# ─────────────────────────────────────────────────────────────────────────────
# Skip guard — skip the whole module if required secrets are absent
# ─────────────────────────────────────────────────────────────────────────────

_REQUIRED = [
    "INTEGRATION_TEST_GITHUB_TOKEN",
    "INTEGRATION_TEST_REPO",
    "INTEGRATION_INSTALLATION_ID",
    "GITHUB_APP_ID",
    "GITHUB_APP_PRIVATE_KEY",
]

_missing = [k for k in _REQUIRED if not os.environ.get(k)]

pytestmark = pytest.mark.skipif(
    bool(_missing),
    reason=(
        f"Integration test skipped — missing env vars: {_missing}. "
        "Set them (or add as GitHub Actions secrets) to run this test."
    ),
)

# ─────────────────────────────────────────────────────────────────────────────
# Read config from env
# ─────────────────────────────────────────────────────────────────────────────

TEST_GITHUB_TOKEN = os.environ.get("INTEGRATION_TEST_GITHUB_TOKEN", "")
TEST_REPO = os.environ.get("INTEGRATION_TEST_REPO", "")  # "owner/name"
INSTALLATION_ID = int(os.environ.get("INTEGRATION_INSTALLATION_ID", "0"))
INGEST_URL = os.environ.get("AEGISDIFF_INGEST_URL", "")
INGEST_TOKEN = os.environ.get("AEGISDIFF_INGEST_TOKEN", "")

GITHUB_API = "https://api.github.com"

# ─────────────────────────────────────────────────────────────────────────────
# Vulnerable code that will be pushed to the test repo
# ─────────────────────────────────────────────────────────────────────────────

VULNERABLE_FILE_PATH = "src/api_handler.py"

VULNERABLE_CODE = """\
\"\"\"API handler for data processing requests.\"\"\"
# NOTE: This file is deliberately vulnerable for AegisDiff integration testing.
import subprocess
import os


def process_request(request_data: dict) -> dict:
    \"\"\"Process an incoming request and return a result.\"\"\"
    # Extract the template name from user input
    template = request_data.get("template", "default")

    # VULNERABILITY (CWE-78): unsanitised user input passed to shell command
    # An attacker can set template = "default; cat /etc/passwd" to exfiltrate data.
    output = subprocess.check_output(
        f"render_template.sh {template}", shell=True
    )
    return {"result": output.decode("utf-8")}


def get_report(query: str) -> str:
    \"\"\"Fetch a report by name.\"\"\"
    # VULNERABILITY (CWE-78): another injection point
    raw = os.popen(f"generate_report {query}").read()
    return raw
"""

# ─────────────────────────────────────────────────────────────────────────────
# GitHub API helpers
# ─────────────────────────────────────────────────────────────────────────────


class GitHubHelper:
    """Thin wrapper around the GitHub REST API for test setup/teardown."""

    def __init__(self, token: str, repo: str) -> None:
        self._headers = {
            "Authorization": f"Bearer {token}",
            "Accept": "application/vnd.github+json",
            "X-GitHub-Api-Version": "2022-11-28",
        }
        self.repo = repo
        self._owner, self._name = repo.split("/", 1)

    def _get(self, path: str, **params) -> dict:
        resp = httpx.get(
            f"{GITHUB_API}{path}",
            headers=self._headers,
            params=params,
            timeout=15.0,
        )
        resp.raise_for_status()
        return resp.json()

    def _post(self, path: str, body: dict) -> dict:
        resp = httpx.post(
            f"{GITHUB_API}{path}",
            headers=self._headers,
            json=body,
            timeout=15.0,
        )
        resp.raise_for_status()
        return resp.json()

    def _patch(self, path: str, body: dict) -> dict:
        resp = httpx.patch(
            f"{GITHUB_API}{path}",
            headers=self._headers,
            json=body,
            timeout=15.0,
        )
        resp.raise_for_status()
        return resp.json()

    def _delete(self, path: str) -> None:
        resp = httpx.delete(
            f"{GITHUB_API}{path}",
            headers=self._headers,
            timeout=15.0,
        )
        if resp.status_code not in (204, 422):
            resp.raise_for_status()

    # ── Repo operations ───────────────────────────────────────────────────────

    def get_default_branch_sha(self) -> tuple[str, str]:
        """Return (default_branch_name, HEAD_sha)."""
        repo_info = self._get(f"/repos/{self.repo}")
        branch = repo_info["default_branch"]
        ref = self._get(f"/repos/{self.repo}/git/ref/heads/{branch}")
        return branch, ref["object"]["sha"]

    def create_branch(self, branch_name: str, from_sha: str) -> None:
        self._post(
            f"/repos/{self.repo}/git/refs",
            {"ref": f"refs/heads/{branch_name}", "sha": from_sha},
        )
        print(f"  → Branch created: {branch_name}")

    def push_file(
        self,
        path: str,
        content: str,
        branch: str,
        message: str,
        existing_sha: Optional[str] = None,
    ) -> str:
        """Create or update a file. Returns the commit SHA."""
        import base64

        body: dict = {
            "message": message,
            "content": base64.b64encode(content.encode()).decode(),
            "branch": branch,
        }
        if existing_sha:
            body["sha"] = existing_sha

        resp = httpx.put(
            f"{GITHUB_API}/repos/{self.repo}/contents/{path}",
            headers=self._headers,
            json=body,
            timeout=15.0,
        )
        resp.raise_for_status()
        commit_sha = resp.json()["commit"]["sha"]
        print(f"  → File pushed: {path}  (commit {commit_sha[:7]})")
        return commit_sha

    def create_pr(
        self, title: str, head: str, base: str, body: str = ""
    ) -> tuple[int, str]:
        """Open a pull request. Returns (pr_number, head_sha)."""
        pr = self._post(
            f"/repos/{self.repo}/pulls",
            {
                "title": title,
                "head": head,
                "base": base,
                "body": body,
                "draft": False,
            },
        )
        pr_number = pr["number"]
        head_sha = pr["head"]["sha"]
        pr_url = pr["html_url"]
        print(f"  → PR #{pr_number} opened: {pr_url}")
        return pr_number, head_sha

    def close_pr(self, pr_number: int) -> None:
        self._patch(
            f"/repos/{self.repo}/pulls/{pr_number}",
            {"state": "closed"},
        )
        print(f"  → PR #{pr_number} closed")

    def delete_branch(self, branch: str) -> None:
        self._delete(f"/repos/{self.repo}/git/refs/heads/{branch}")
        print(f"  → Branch deleted: {branch}")

    # ── Verification helpers ──────────────────────────────────────────────────

    def get_pr_comments(self, pr_number: int) -> list[dict]:
        return self._get(
            f"/repos/{self.repo}/issues/{pr_number}/comments",
            per_page=50,
        )

    def get_commit_statuses(self, sha: str) -> list[dict]:
        return self._get(f"/repos/{self.repo}/commits/{sha}/statuses")

    def find_aegisdiff_comment(self, pr_number: int) -> Optional[dict]:
        for comment in self.get_pr_comments(pr_number):
            if "<!-- aegisdiff-report -->" in comment.get("body", ""):
                return comment
        return None

    def find_aegisdiff_status(self, sha: str) -> Optional[dict]:
        for status in self.get_commit_statuses(sha):
            if status.get("context") == "AegisDiff / security":
                return status
        return None

    def poll_for_comment(
        self, pr_number: int, timeout: int = 120, interval: int = 5
    ) -> Optional[dict]:
        """Poll until AegisDiff comment appears or timeout expires."""
        deadline = time.monotonic() + timeout
        while time.monotonic() < deadline:
            comment = self.find_aegisdiff_comment(pr_number)
            if comment:
                return comment
            time.sleep(interval)
        return None


# ─────────────────────────────────────────────────────────────────────────────
# The integration test
# ─────────────────────────────────────────────────────────────────────────────


class TestRealGitHubPRScan:
    """
    Full end-to-end integration test using real GitHub API calls.

    Timeline:
      ~0s   Create branch + push vulnerable code + open PR
      ~5s   Run app_entrypoint.main() (real GitHub App token + real LLM)
      ~30s  LLM returns verdict, PR comment posted, commit status set
      ~35s  Ingest called, assertions pass
      ~36s  Cleanup: PR closed, branch deleted
    """

    @pytest.fixture(autouse=True)
    def _setup_and_teardown(self):
        """Create the test branch/PR before the test, clean up after."""
        suffix = "".join(random.choices(string.ascii_lowercase + string.digits, k=6))
        self.branch = f"aegisdiff-integration-test-{suffix}"
        self.gh = GitHubHelper(TEST_GITHUB_TOKEN, TEST_REPO)
        self.pr_number: Optional[int] = None
        self.head_sha: Optional[str] = None

        yield  # test runs here

        # Always clean up — even if the test fails
        print("\n  [cleanup]")
        if self.pr_number:
            try:
                self.gh.close_pr(self.pr_number)
            except Exception as e:
                print(f"  ⚠ Could not close PR: {e}")
        try:
            self.gh.delete_branch(self.branch)
        except Exception as e:
            print(f"  ⚠ Could not delete branch: {e}")

    def test_pr_scan_detects_command_injection(self):
        """
        Full pipeline:
          1. Push CWE-78 vulnerable code to a real PR
          2. Run AegisDiff engine (real LLM call)
          3. Verify PR comment + commit status + dashboard ingest
        """

        print("\n")
        print("=" * 70)
        print("  AegisDiff — Real-GitHub Integration Test")
        print(f"  Target repo : {TEST_REPO}")
        print("=" * 70)

        # ── Step 1: Create branch + push vulnerable file ─────────────────────
        print("\n  [1] SETTING UP TEST PR")
        default_branch, base_sha = self.gh.get_default_branch_sha()
        print(f"  → Base branch: {default_branch}  ({base_sha[:7]})")

        self.gh.create_branch(self.branch, base_sha)

        commit_sha = self.gh.push_file(
            path=VULNERABLE_FILE_PATH,
            content=VULNERABLE_CODE,
            branch=self.branch,
            message=(
                "feat: add API handler with data processing\n\n"
                "[aegisdiff-integration-test] This commit is intentionally "
                "vulnerable for CI testing."
            ),
        )
        self.head_sha = commit_sha

        pr_number, head_sha = self.gh.create_pr(
            title=f"[AegisDiff CI] Integration test — add API handler ({self.branch[-6:]})",
            head=self.branch,
            base=default_branch,
            body=(
                "**This is an automated integration test PR created by AegisDiff CI.**\n\n"
                "It contains a deliberately vulnerable Python file to verify that "
                "AegisDiff correctly detects and reports OS command injection (CWE-78).\n\n"
                "_This PR will be closed automatically at the end of the test run._"
            ),
        )
        self.pr_number = pr_number
        # Use the PR head SHA (most reliable for status API)
        self.head_sha = head_sha

        print(f"\n  → head_sha = {head_sha[:7]}")
        print(f"  → PR #{pr_number} on {TEST_REPO}")

        # ── Step 2: Run AegisDiff engine ─────────────────────────────────────
        print("\n  [2] RUNNING AEGISDIFF ENGINE")
        print("  → Calling app_entrypoint.main() with real credentials...")

        owner, repo_name = TEST_REPO.split("/", 1)

        # Set all env vars that main() reads via _get_env() / os.environ.get()
        env_overrides = {
            "GITHUB_APP_ID": os.environ["GITHUB_APP_ID"],
            "GITHUB_APP_PRIVATE_KEY": os.environ["GITHUB_APP_PRIVATE_KEY"],
            "INSTALLATION_ID": str(INSTALLATION_ID),
            "TARGET_REPO": TEST_REPO,
            "PR_NUMBER": str(pr_number),
            "COMMIT_SHA": head_sha,
            "GITHUB_TOKEN": os.environ.get("GITHUB_TOKEN", ""),
            # User LLM keys — use from env if available, else rely on GitHub Models
            "OPENROUTER_API_KEY": os.environ.get("OPENROUTER_API_KEY", ""),
            "OPENROUTER_API_KEY_2": os.environ.get("OPENROUTER_API_KEY_2", ""),
            "OPENROUTER_API_KEY_3": os.environ.get("OPENROUTER_API_KEY_3", ""),
            "GROQ_API_KEY": os.environ.get("GROQ_API_KEY", ""),
            "GROQ_API_KEY_2": os.environ.get("GROQ_API_KEY_2", ""),
            "GROQ_API_KEY_3": os.environ.get("GROQ_API_KEY_3", ""),
            "AEGISDIFF_INGEST_URL": INGEST_URL,
            "AEGISDIFF_INGEST_TOKEN": INGEST_TOKEN,
        }

        # Capture what was sent to the dashboard for later assertion
        ingest_captured: dict = {}

        original_send = None

        def capturing_send(ingest_url, ingest_token, verdicts, pr_num, sha, repo, ms):
            """Intercept _send_to_ingest to capture payload while still calling through."""
            from aegisdiff.app_entrypoint import _build_item
            from aegisdiff.triage.verdicts import Verdict as _Verdict

            if isinstance(verdicts, _Verdict):
                payload = _build_item(verdicts, pr_num, sha, repo, ms)
            else:
                payload = [_build_item(v, pr_num, sha, repo, ms) for v in verdicts]

            ingest_captured["payload"] = payload
            ingest_captured["url"] = ingest_url
            print(
                f"\n  → Ingest payload captured: "
                f"verdict={payload['verdict'] if isinstance(payload, dict) else payload[0]['verdict']}"
            )
            # Call through to the real endpoint if configured
            if original_send and ingest_url:
                original_send(ingest_url, ingest_token, verdicts, pr_num, sha, repo, ms)

        scan_start = time.monotonic()
        exit_code = None

        with patch.dict(os.environ, env_overrides, clear=False):
            # Capture ingest without blocking it
            import aegisdiff.app_entrypoint as _ep

            original_send = _ep._send_to_ingest
            with patch.object(_ep, "_send_to_ingest", side_effect=capturing_send):
                try:
                    # Reload to pick up fresh env
                    _ep.main()
                    exit_code = 0
                except SystemExit as e:
                    exit_code = e.code

        scan_ms = int((time.monotonic() - scan_start) * 1000)
        print(f"\n  → Engine finished in {scan_ms}ms, exit code = {exit_code}")

        # ── Step 3: Verify results ────────────────────────────────────────────
        print("\n  [3] VERIFYING RESULTS")

        # 3a — PR comment
        print("  → Checking PR comment...")
        comment = self.gh.find_aegisdiff_comment(pr_number)
        assert comment is not None, (
            f"AegisDiff did not post a PR comment on {TEST_REPO}#{pr_number}. "
            "Check that the GitHub App installation token has PR write permission."
        )
        comment_body = comment["body"]
        print(f"  ✓ PR comment posted  (id={comment['id']})")
        print(f"    First line: {comment_body.split(chr(10))[0]}")

        # Comment must contain the AegisDiff marker
        assert "<!-- aegisdiff-report -->" in comment_body

        # The vulnerable code has two clear CWE-78 sinks — comment should say TRUE_POSITIVE
        verdict_in_comment = (
            "TRUE_POSITIVE" in comment_body
            or "true_positive" in comment_body.lower()
            or "True Positive" in comment_body
        )
        assert verdict_in_comment, (
            f"Expected TRUE_POSITIVE verdict in PR comment, got:\n{comment_body[:500]}"
        )
        print("  ✓ Verdict in comment: TRUE_POSITIVE")

        # 3b — Commit status
        print("  → Checking commit status...")
        status = self.gh.find_aegisdiff_status(head_sha)
        assert status is not None, (
            f"AegisDiff did not post a commit status on {head_sha[:7]}. "
            "Check that the GitHub App has 'commit statuses: write' permission."
        )
        print(f"  ✓ Commit status posted")
        print(f"    State   : {status['state']}")
        print(f"    Desc    : {status['description']}")
        print(f"    Context : {status['context']}")

        assert status["context"] == "AegisDiff / security"
        # TRUE_POSITIVE → "failure" (blocks merge if branch protection is enabled)
        assert status["state"] == "failure", (
            f"Expected 'failure' commit status for TRUE_POSITIVE, got '{status['state']}'"
        )

        # 3c — Dashboard ingest
        print("  → Checking dashboard ingest...")
        assert ingest_captured, "Ingest function was never called"

        payload = ingest_captured["payload"]
        item = payload[0] if isinstance(payload, list) else payload

        assert item["verdict"] == "TRUE_POSITIVE"
        assert "CWE-78" in item["cwe_id"], (
            f"Expected CWE-78 in ingest payload, got: {item['cwe_id']}"
        )
        assert item["pr_number"] == pr_number
        assert item["commit_sha"] == head_sha
        assert item["pr_url"] == f"https://github.com/{TEST_REPO}/pull/{pr_number}"
        assert item["scan_ms"] > 0

        print(f"  ✓ Ingest payload valid")
        print(f"    verdict   = {item['verdict']}")
        print(f"    cwe_id    = {item['cwe_id']}")
        print(f"    severity  = {item['severity']}")
        print(f"    confidence= {item['confidence']:.0%}")
        print(f"    provider  = {item['provider']}")
        print(f"    scan_ms   = {item['scan_ms']}ms")

        # 3d — Exit code (TRUE_POSITIVE with high confidence → blocks merge)
        if exit_code == 1:
            print(f"  ✓ Exit code 1 — high-confidence TRUE_POSITIVE blocks merge")
        else:
            print(f"  ✓ Exit code {exit_code}")

        # ── Final summary ─────────────────────────────────────────────────────
        print("\n" + "=" * 70)
        print("  INTEGRATION TEST PASSED")
        print(f"  PR #{pr_number} on {TEST_REPO}")
        print(f"  Verdict: TRUE_POSITIVE  •  CWE: {item['cwe_id']}")
        print(f"  Scan time: {scan_ms}ms  •  Provider: {item['provider']}")
        print(f"  Comment posted: ✓  |  Commit status: {status['state']}  |  Ingest: ✓")
        print("=" * 70 + "\n")
